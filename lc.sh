...
do
  ...
  if [ -n "$tag" ];then
    bdy=$(echo "$bdy" | sed -i '' -e "s/@[due|remind|reminded][^)]*)/$tag/g" -e 's/^/[NewTag] /')
  fi
...
lcdonerun )
echo "Archived:"
sed -n 's/^x //gp' "$todotxt"
grep '^x ' "$todotxt" | sed 's/@[due|remind][^)]*)//g' >> "$donetxt"
sed -i '' '/^x /d' "$todotxt"
sed -n '$p' "$donetxt" | (sed -i '' '$d' "$donetxt"; xargs echo -n >> "$donetxt")
sed -n '$p' "$todotxt" | (sed -i '' '$d' "$todotxt"; xargs echo -n >> "$todotxt")
;;
