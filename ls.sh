...
if [ echo ${dat}${tim} | grep '[0-9]' ];then
  [ echo $dat | grep -q '^+[0-9]\+$' ];RP=$?
  [ echo $dat | grep -q '^[0-9]\{1,2\}$' ];RD=$?
  [ echo $dat | grep -q '^[0-9]\{1,2\}-[0-9]\{1,2\}$' ];RM=$?
  [ echo $dat | grep -q '^[0-9]\{4\}-[0-9]\{1,2\}-[0-9]\{1,2\}$' ];RY=$?
  case "${RP}${RD}${RM}${RY}" in
    0111)
